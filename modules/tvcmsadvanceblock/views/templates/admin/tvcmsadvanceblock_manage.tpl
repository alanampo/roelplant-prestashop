...
<input type="hidden" name="tvcmsinstall_tab_number" id='tvcmsinstall-tab-number'>
    <form id="module_form" action="{$tvurlupgrade}" method="post" enctype="multipart/form-data" novalidate="">
        <button class="btn btn-primary tvcms-sample-install-btn" type="submit" onclick="return confirm('Are you sure want to install sample data?')" name="submitTvcmsSampleinstall">{l s='Install Sample Data' mod="tvcmsadvanceblock"}</button>
    </form>
<div class="seperation defaultHidden"> </div>
<table id="table-data" class="table tvcmstable tvcmsadvanceblock">
	<thead>
		<tr class="nodrag nodrop">
			{if $show_fields.image == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Image' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}

			{if $show_fields.title == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Title' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}
			...
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Short Description' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}

			{if $show_fields.description == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Description' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}

			{if $show_fields.link == true}
			<th class="fixed-width-xs center">
				<span class="title_box active">{l s='Link' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}

			{if $show_fields.status == true}
			<th class="fixed-width-xs center status">
				<span class="title_box active">{l s='Status' mod='tvcmsadvanceblock'}</span>
			</th>
			{/if}

			<th class="fixed-width-xs center action">
				<span class="title_box active">{l s='Action' mod='tvcmsadvanceblock'}</span>
			</th>
		</tr>	
	</thead>
			...
			<tr id="recordsArray_{$Data.id|escape:'htmlall':'UTF-8'}">
				...
				<td class="pointer fixed-width-xs center">
					<form action="" class="actionEdit" name="actionEdit" method="post">
						...
						<button title="Edit" class="edit" onclick="$(this).parent('.actionEdit').trigger('submit');" ><i class="icon-pencil"></i> {l s='Edit' mod='tvcmsadvanceblock'} </button>						
					</form>
					<form  action="" class="actionDelete" name="actionDelete" method="post"> 
						<input type="hidden" name="id" value="{$Data.id|escape:'htmlall':'UTF-8'}">		
						<input type="hidden" name="action" value="remove">		
						<button title="Delete" class="delete tvcmsdelete"><i class="icon-trash"></i> {l s='Delete' mod='tvcmsadvanceblock'} </button>
					</form>
				</td>
			</tr>
		{/foreach}
