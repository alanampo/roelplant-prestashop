...
function upgrade_module_1_1_7($module)
{
    ...
    try {
        ...
        Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'gptcontent_post_category`');
        Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'gptcontent_post_category_lang`');

        $sqlInstaller = new Installer();
        $sqlInstaller->installBlogCategories();
        $sqlInstaller->installBlogHooks();
        $sqlInstaller->installPostsCron();
