...
$(document).on('ready', function () {
  $('.shortcod-value').on('click', function () {
    ...
    var textarea = $(this).closest('.prompt-short-code').find('.lang-' + id_language + ' textarea')[0];
    ...
    displayLanguagesPrompt();
  })

  $('.prompt-short-code textarea').on('change', function () {
    displayLanguagesPrompt();
  })
...
function displayLanguagesPrompt() {
  $('.prompt-short-code').each(function() {
    let languagesPompt = [];

    $(this).find('textarea').each(function () {
      if ($(this).val()) {
        let id_lang = parseInt($(this).attr('name').match(/\d+/));
        languagesPompt.push(getLanguageNameById(id_lang));
      }
    });

    $(this).find('.help-block-prompt-languages').remove();
    ...
    if (languagesPompt.length > 0) {
      let htmlLanguages =
        ...
        $(this).find('.help-block').prepend(htmlLanguages);
    }
  })
}
...
function checkType() {
  if ('custom' == $('select[name=type]').val()) {
    $('.prompt-short-code').slideDown();
  } else {
    $('.prompt-short-code').slideUp();
  }
}
