		...
		<ul>
			...
			<li>
				...
				<h6>
                    ...
                    {if isset($product.reference) AND $product.reference}<br><small class="form-control-comment text-muted hint">({$product.reference|escape:'htmlall':'UTF-8'})</small>{/if}
                </a>
    ...
    <thead>
        <tr>
            ...
            <th>{l s='Valid Till' mod='productquotation'}</th>
             <th>{l s='Discount Value' mod='productquotation'}</th>
...
<br />
<h3 class="page-subheading">{l s='Extra Chargers' mod='productquotation'}</h3>
<table class="table table-bordered">
    <thead>
        <tr>
            <th>{l s='ID' mod='productquotation'}</th>
            <th>{l s='Charges' mod='productquotation'}</th>
            <th>{l s='Code' mod='productquotation'}</th>
        </tr>
    </thead>
    <tbody>
            <tr>
                <td>{$voucher.coupon_id|escape:'htmlall':'UTF-8'}</td>
                <td>{FieldsQuote::getFeeProductIdPrice($id_quotation)} {$currencyy}</td>
                <td>{$voucher.code|escape:'htmlall':'UTF-8'}</td>
            </tr>
    </tbody>
</table>
...
<ul class="footer_links clearfix">
    ...
    <li>
	 ...
	 <li style="float: right">
        {if isset($id_quote)}
        <a class="btn btn-default button button-small btn-info" href="{$link->getModuleLink('productquotation', 'quotations', ['pdf' => 1, 'id_quote' => $id_quote, 'id_quotation' => $id_quotation], true)|escape:'htmlall':'UTF-8'}">{l s='Download PDF' mod='productquotation'} <i class="material-icons">cloud_download</i>
        </a>
        {/if}
        <a href="{$link->getModuleLink('productquotation', 'messages')|escape:'htmlall':'UTF-8'}?id_quotation={$id_quotation|escape:'htmlall':'UTF-8'}&key={$key|escape:'htmlall':'UTF-8'}" class="btn btn-default button button-small btn-primary">
            <span>
