...
{/if}
    <form action="{$form_action|escape:'htmlall':'UTF-8'}" enctype="multipart/form-data" method="post">
        ...
        <div class="submit col-md-12" style="clear: both">
            <button class="btn button button-primary" id="submitMessage" name="submitMessage" type="submit"><span>{l s='Send' mod='productquotation'} <i class="icon-chevron-right right"></i></span></button>
        </div>
    </form>
{/if}
...
<ul class="footer_links clearfix">
    <li>
        <a href="{$link->getModuleLink('productquotation', 'quotations', [], true)|escape:'htmlall':'UTF-8'}" class="btn button button-info">
            <span>
    ...
    <li>
        <a href="{$link->getPageLink('my-account')|escape:'htmlall':'UTF-8'}" class="btn button button-primary">
            <span>
