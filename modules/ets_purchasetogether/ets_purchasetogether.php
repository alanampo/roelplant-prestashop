...
if(!class_exists('AjaxPurchaseTogether'))
...
class Ets_purchasetogether extends Module 
{
    ...
    public function runAjax()
    {
        if(Tools::getIsset('add')){
            AjaxPurchaseTogether::ajaxAdd();
        }elseif(Tools::getIsset('del')){
            AjaxPurchaseTogether::ajaxDelete();
        }elseif(Tools::getIsset('q')){
            AjaxPurchaseTogether::ajaxSearch();           
        }else
            die('error');           
    }            
    //suffix
	...
	public function assignContentVars($params)
	{
		...
		{
			$total_free_shipping =  (float)($free_shipping - ($params['cart']->getOrderTotal(true, Cart::ONLY_PRODUCTS) +
     ...
     public function hookDisplayAdminProductsExtra($param)
     {
        ...
        if (($product = new ptProduct($id_product)) && Validate::isLoadedObject($product))
        {
            ...
            $this->smarty->assign(array(
              ...
              'url_ajax' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name,
              'is17' =>$this->is17,
