    ...
    {if isset($purchase_togethers) && $purchase_togethers && count($purchase_togethers) != $alldisabled}
        ...
        {block name="purchase_together"}
            ...
            {if (isset($configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT) && !$configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT) 
                ...
                <li class="item-product product-miniature">
                    ...
                    <div class="ets_purchase_item_des">
                        <span class="h3 product-title"><a class="purchase_current_product">{l s='This current product' mod='ets_purchasetogether'} (<b>{$currProduct.name|escape:'html':'UTF-8'}</b>)</a></span>
                                ...
                                <span class="product-price-and-shipping">
                                    {*if $currProduct.has_discount}
                                        {if isset($configs.ETS_PT_DISPLAY_OLD_PRICE) && $configs.ETS_PT_DISPLAY_OLD_PRICE}
                                            ...
                                            <span class="regular-price">{$currProduct.regular_price|floatval}</span>
                                        {/if}
                                    ...
                                    <span itemprop="price" class="price">{$currProduct.price|floatval}</span>
                        ...
                        <div class="row-product">
                            ...
                            <div class="ets_purchase_item_des">
                                <span class="h3 product-title">
                                      <a href="{$product.url|escape:'html':'UTF-8'}">{$product.name_attribute|truncate:100:'...'|escape:'html':'UTF-8'}</a>
                                </span>
                                {block name='product_price_and_shipping'}
                                    {if $product.show_price && isset($configs.ETS_PT_DISPLAY_PRODUCT_PRICE) && $configs.ETS_PT_DISPLAY_PRODUCT_PRICE}
                                        <span class="product-price-and-shipping">
                                            ...
                                            <span itemprop="price" class="price">{$product.price|floatval}</span>
        	...
        	<div class="button-container {if count($purchase_togethers) == $alldisabled}disabled{/if}">
        		<a class="ets_ajax_add_to_cart_button btn btn-primary" href="{$ajax_cart|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add all products to cart' mod='ets_purchasetogether'}">
                ...
                {if (isset($configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT) && !$configs.ETS_PT_EXCLUDE_CURRENT_PRODUCT)
                    ...
                    <li class="item-product product-miniature">
                        <div class="row-product">
                            <div class="ets_purchase_item_image">
                                ...
                                {block name='product_thumbnail'}
                                    <a href="{$currProduct.url|escape:'html':'UTF-8'}" class="thumbnail product-thumbnail">
                                      <img
                                        src = "{$currProduct.cover.bySize.medium_default.url|escape:'html':'UTF-8'}"
                                        alt = "{$currProduct.cover.legend|escape:'html':'UTF-8'}"
                                        data-full-size-image-url = "{$currProduct.cover.medium.url|escape:'html':'UTF-8'}"
                                      >
                                    </a>
                                {/block}     
                            </div>
                            <div class="ets_purchase_item_des">
                                <span class="h3 product-title">
                                    <a class="purchase_current_product">{l s='This product' mod='ets_purchasetogether'} (<b>{$currProduct.name|escape:'html':'UTF-8'}</b>)</a>
                                </span>
                                {block name='product_price_and_shipping'}
                                    {if $currProduct.show_price && isset($configs.ETS_PT_DISPLAY_PRODUCT_PRICE) && $configs.ETS_PT_DISPLAY_PRODUCT_PRICE}
                                        <span class="product-price-and-shipping">
                                        ...
                                        {if $product.show_price && isset($configs.ETS_PT_DISPLAY_PRODUCT_PRICE) && $configs.ETS_PT_DISPLAY_PRODUCT_PRICE}
                                            <span class="product-price-and-shipping">
                                                {if $product.has_discount}
                                                    {if isset($configs.ETS_PT_DISPLAY_OLD_PRICE) && $configs.ETS_PT_DISPLAY_OLD_PRICE}
                                                        {hook h='displayProductPriceBlock' product=$product type="old_price"}
                                                        <span class="regular-price">{$product.regular_price|floatval}</span>
                                                    {/if}
                                                    {if $product.discount_type === 'percentage' && isset($configs.ETS_PT_DISPLAY_DISCOUNT) && $configs.ETS_PT_DISPLAY_DISCOUNT}
                                                      <span class="discount-percentage">{$product.discount_percentage|escape:'html':'UTF-8'}</span>
                                                    {/if}
