...
class AjaxPurchaseTogether extends Module
{   
    ...
    public static function ajaxSearch()
    {
        ...
        $imageType = version_compare(_PS_VERSION_, '1.7.0', '>=')?ImageType::getFormattedName('cart'):ImageType::getFormatedName('cart'); 
        ob_end_clean();
                        ...
                        if (!empty($combinations)) 
                        {
                            foreach ($combinations as $combination) 
                            {
                                ...
                                if (empty($results[$combination['id_product_attribute']]['image'])) {
                                    $results[$combination['id_product_attribute']]['image'] = !empty($combination['id_image'])?str_replace('http://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $combination['id_image'], $imageType)): str_replace('http://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $item['id_image']));
                                }
                            }
                        } 
                        else 
