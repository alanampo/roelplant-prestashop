...
class TvcmsCustomSetting extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('JHP Template - Custom Setting');
        $this->description = $this->l('It is use of Custom Setting in JHPTemplate Theme');
    ...
    {
        if (!(int) Tab::getIdFromClassName('AdminJHPTemplate')) {
            $parent_tab = new Tab();
            // Need a foreach for the language
            foreach (Language::getLanguages() as $language) {
                $parent_tab->name[$language['id_lang']] = $this->l('JHPTemplate Extension');
            }
            $parent_tab->class_name = 'AdminJHPTemplate';
        ...
        $tab->id_parent = (int) Tab::getIdFromClassName('AdminJHPTemplate');
    ...
    public function createDefaultData()
    {
        ...
        foreach ($languages as $lang) {
            ...
            $result['TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE'][$lang['id_lang']] = 'Get Offers';
        ...
        Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE', 'demo_img_2.jpg');
        ...
        Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS', 0);
        ...
        $tmp = $result['TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE'];
        Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE', $tmp);
    ...
    public function deleteVariable()
    {
        ...
        Configuration::deleteByName('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE');
        ...
        Configuration::deleteByName('TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE');
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitTvcmsCustomTitleForm')) {
            if ($_FILES['TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE']) {
                $old_img = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE');
                $this->obj_image = new TvcmsCustomSettingImageUpload();
                $ans = $this->obj_image->imageUploading($_FILES['TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE'], $old_img);
                if ($ans['success']) {
                    $file_name = $ans['name'];
                    Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE', $file_name);
                } else {
                    $old_img = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE');
                    Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE', $old_img);
                }
            }
        }
        ...
        if (Tools::isSubmit('submitTvcmsCustomTitleForm') && Tools::getValue('tvinstalldata') == "0") {
            foreach ($languages as $lang) {
                ...
                $tmp = Tools::getValue('TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE_'.$lang['id_lang']);
                $result['TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE'][$lang['id_lang']] = $tmp;
            ...
            $tmp = Tools::getValue('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS');
            if ($tmp != Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS')) {
                Configuration::updateValue('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS', $tmp);
            }
    ...
    protected function tvcmsFooterProductForm()
    {
        ...
        if (Module::isInstalled('ps_emailsubscription')) {
            if ($show_fields['news_letter_sub_title']) {
                $input[] = array(
                        'col' => 9,
                        'type' => 'text',
                        'name' => 'TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE',
                        'label' => $this->l('Newsletter Sub Title'),
                        'desc' => $this->l('Display Newsletter Sub Title in Front Side'),
                        'lang' => true,
                    );
            }
        ...
        if ($show_fields['newsletter_img']) {
            $input[] = array(
                        'col' => 9,
                        'type' => 'newsletter_img',
                        'name' => 'TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE',
                        'label' => $this->l('Newsletter Background Image'),
                    );
        }
        if ($show_fields['news_letter_img_status']) {
                $input[] =  array(
                        'type' => 'switch',
                        'label' => $this->l('Newsletter Background Image Status'),
                        'name' => 'TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS',
                        'desc' => $this->l('Manage Newsletter Background image Status'),
                        'is_bool' => true,
                        'values' => array(
                            array(
                                'id' => 'active_on',
    ...
    protected function getConfigFormValues()
    {
        ...
        foreach ($languages as $lang) {
            ...
            $a = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE', $lang['id_lang']);
            $fields['TVCMSCUSTOMSETTING_NEWSLETTER_SUB_TITLE'][$lang['id_lang']] = $a;
        ...
        $tmp = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE');
        $fields['TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE'] = $tmp;
        ...
        $tmp = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS');
        $fields['TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS'] = $tmp;
    ...
    public function hookdisplayTopOfferText()
    {
        ...
        if (!Cache::isStored('tvcmscustomsetting_displaytopoffertext')) {
            if (Configuration::get('TVCMSCUSTOMSETTING_CUSTOM_TEXT_STATUS')) {
                $html .= '<div class="tvheader-nav-offer-text"><i class=\'material-icons\'>local_offer</i>' . Configuration::get('TVCMSCUSTOMSETTING_CUSTOM_TEXT', $id_lang, true) . '</div>';
            }
            $output = $html;
    ...
    public function hookdisplayHeader()
    {
        ...
        $NewsletterImageStatus = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_BACK_IMG_STATUS');
        $this->context->smarty->assign("NewsletterImageStatus", $NewsletterImageStatus);
        ...
        $Newsletterpath = _MODULE_DIR_.$this->name."/views/img/";
        $this->context->smarty->assign('Newsletterpath', $Newsletterpath);
        
        $NewsletterBackgroundImage = Configuration::get('TVCMSCUSTOMSETTING_NEWSLETTER_IMAGE');
        $this->context->smarty->assign("NewsletterBackgroundImage", $NewsletterBackgroundImage);
